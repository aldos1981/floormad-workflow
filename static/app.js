...
// Init Logic
function initApp() {
    if (dbg) dbg.innerHTML += '<div class="text-yellow-400">[TRACE] initApp Called</div>';
    console.log("DOM Ready. Initializing...");

    // Fetch Projects
    fetchProjects().then(() => {
        if (dbg) dbg.innerHTML += '<div class="text-green-400">[TRACE] fetchProjects Promise Resolved</div>';
    }).catch(err => {
        if (dbg) dbg.innerHTML += '<div class="text-red-400">[TRACE] fetchProjects Failed</div>';
